...
int main() {
    auto exp_model = [](double x, const std::vector<double>& p) {
    ...
    std::vector<double> true_p(3);
    true_p = {2.0, -0.3, 0.5};
    for (double xi : xdata) {
        double yi = exp_model(xi, true_p) + 0.05 * ((std::rand() / (double)RAND_MAX) - 0.5);
        ydata.push_back(yi);
    }

    std::vector<double> p0 = {1.0, -0.1, 0.0}; // initial guess
